...
function App() {
  ...
  async function onSave() {
    ...
    const listName = listNameById.get(selectedListId) ?? "Unknown";
    setSaving(true);
    try {
      const rule: CloneRule = { id: uid(), srcId: selectedCard.id, srcName: selectedCard.name, listId: selectedListId, listName, repeat, weekday, dayOfMonth: parseInt(dayOfMonth, 10), time: atTime || "09:00", pos: position, expiry: computeExpiry(expiry), active: true, lastRun: null, created: new Date().toISOString() };
      const api = await t.getRestApi(); if (!(await api.isAuthorized())) await api.authorize({ scope: "read,write", expiration: "never" });
      await api.post("cards", { idCardSource: selectedCard.id, idList: selectedListId, pos: position === "Top" ? "top" : "bottom", keepFromSource: "attachments,checklists,comments,labels,members,stickers" });
            ...
            <>
              ...
              {ctx === "board" && (
                <SelectField
                  ...
                  onChange={(name) => { const li = lists.find((l) => l.name === name); if (li) { setSelectedListId(li.id); setSelectedCardId(""); setCardQuery(""); } }}
                />
              )}
